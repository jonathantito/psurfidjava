public class ByteAndHexConvertingUtility {
	...
	/**
	 * This method converts a 32-bit integer to a byte array, MSB first. It drops the
	 * leading zeros in the int, unless the minSize is greater than the number
	 * of leading non-zero bytes
	 * 
	 * @param num
	...
	public static byte[] intToByteArray(int num, int minSize) {
		...
		if(1<=numbits && numbits<=8) numOfIncomingBytes = 1;
		else if (9<=numbits && numbits<=16) numOfIncomingBytes = 2;
		else if (17<=numbits && numbits<=24) numOfIncomingBytes = 3;
		else if (25<=numbits && numbits<=32) numOfIncomingBytes = 4;
	...
	/**
	 * Convert 32-bit integer to byte array, MSB first. This method will drop the leading
	 * zeros
	 * 
	 * @param num
	 *            The int to convert
